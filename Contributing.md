...
* GitHub の問題を使用してドキュメントのバグを報告します。
...
2. アカウントを取得したら、コンピューターに Git をインストールします。 [Git の設定]のチュートリアルの手順に従います。
...
署名、干渉しない権利を付与する、メインのリポジトリにコミットするが、Office 開発者および Office 開発コンテンツ作成チームを確認し、あなたの投稿を承認することになることだといいます。 提出書類の貸方記入されます。
...
2.  Git をコンピューターにインストールします。 [Git の設定]のチュートリアルの手順に従います。
3.  このリポジトリの独自のフォークを作成します。 ページの上部にある、[**分岐**] ボタンを選択します。
...
このリポジトリ内のすべての記事では、Markdown を使用しています。 [Daring ファイアボールの値下げ]では、完全な概要 (およびすべての構文の一覧) を参照しています。
...
* 値下げに関する詳細については、するには、[ファイアボールの Daring]の値下げの作成元のサイトに移動します。
* Git と GitHub を使用する方法の詳細については、まず[GitHub のヘルプ]を確認します。
